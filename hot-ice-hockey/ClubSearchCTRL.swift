...
class ClubSearchCTRL: UITableViewController,UISearchBarDelegate {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        tracker.set(kGAIScreenName, value: "ClubSearchCTRL")
    ...
    func searchClubs() -> Bool {
        ...
        var tracker = GAI.sharedInstance().defaultTracker
        var builder = GAIDictionaryBuilder.createScreenView()
        var event = GAIDictionaryBuilder.createEventWithCategory("API Requests", action: "GET", label: "Search", value: nil)
        tracker.send(event.build() as [NSObject : AnyObject])
        
        Loading.start()
        
        let s = "https://www.easports.com/iframe/nhl14proclubs/api/platforms/\(platforms[Tools.rd()!])/clubsComplete/\(searchBar.text)"
